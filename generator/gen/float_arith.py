...
from ..common import f32_to_bits as _f32b, f64_to_bits as _f64b
...
def generate(base_dir: Path) -> list[str]:
    ...
    for mnemonic, cfn in _FP_FMA_VV:
        ...
        for sew in FP_SEWS:
            ...
            if sew == 32:
                ...
                mask_bits = 0b1010
                exp_full = [
                    cfn(d, s1, s2, sew) for d, s1, s2 in zip(vd_init_m, vs1_ov, vs2_m)
                ]
                exp_masked = [
    ...
    for mnemonic, cfn in _FP_CMP_VF:
        ...
        for sew in FP_SEWS:
            ...
            # Masked compare VF: mask=0b0101
            mask_bits = 0b0101
            vd_init_mask = 0b1010
            b = _f32b if sew == 32 else _f64b
            s2_m = [b(1.0), b(2.0), b(3.0), b(4.0)]
            sc_m = b(2.5)
            expected_mask = 0
            for i in range(4):
                if (mask_bits >> i) & 1:
                    if cfn(s2_m[i], sc_m, sew):
                        expected_mask |= 1 << i
                else:
                    if (vd_init_mask >> i) & 1:
                        expected_mask |= 1 << i
